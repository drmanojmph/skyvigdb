...
    if (selected.currentStep < 3) { flash(" E2B R3 XML is available from Medical Review (Step 3) onward.", "error"); return; }
    try {
...
        );
        if (dup && !silent) { alert(" Possible duplicate: " + dup.caseNumber); return true; }
        return false;
      }
...
        _audit: { performedBy: user.username, role: user.role }
      });
      setSelected(null); setForm({}); fetchCases(); flash(" Case returned to Data Entry.");
...
        _audit: { performedBy: user.username, role: user.role }
      });
      setSelected(null); setForm({}); fetchCases(); flash(" Case returned to Quality Review.");
    } catch { flash(" Routing failed.", "error"); }
  };
...
    CASE_UPDATED:        { color:"bg-indigo-100 text-indigo-800",icon:" ", label:"Case Updated" },
...
    CASE_RETURNED:       { color:"bg-amber-100 text-amber-800",  icon:" ", label:"Returned for Query" },
    ROUTE_BACK_TO_DE:    { color:"bg-amber-100 text-amber-800",  icon:" ", label:"Returned to Data Entry" },
    RETURNED_TO_MEDICAL: { color:"bg-amber-100 text-amber-800",  icon:" ", label:"Returned to Medical" },
...
          {dupResults.length > 0 ? (
            <>
              <div className="font-extrabold text-amber-800 mb-2"> {dupResults.length} potential duplicate{dupResults.length>1?"s":""} found</div>
...
                  Case classified as SERIOUS based on criteria selected or IME term detected.
                </div>
              )}
            </div>
...
                      IME term detected — this case MUST be classified as SERIOUS
                    </div>
                  )}
...
            <span className="text-xs font-extrabold text-amber-800 uppercase tracking-widest"> No Narrative Submitted</span>
...
                  {F("Causality Method", S(["WHO-UMC","Naranjo"], { value:e.causalityMethod||"", onChange:ev => setEv(idx,"causalityMethod",ev.target.value) }))}
                </div>
...
              ["returned"," Return to Medical","bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 shadow-orange-200"]
            ].map(([val2,lbl7,cls]) => (
...
        Read-only — this case is at <strong>{selected.status}</strong> (Step {selected.currentStep}). Your role: <strong>{user.role}</strong> (Step {user.step}).
      </div>
...
              No agency submissions recorded. Ensure all required submissions are logged in Step 5 before archiving.
            </div>
          )}
        </div>
...
                     selected.currentStep === 6 ? " Archive & Close →" :
                     "Submit →"}
                  </button>
                )}
